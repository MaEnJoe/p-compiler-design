...
std::vector<PTypeSharedPtr>& FunctionNode::getPrototypePTypeVec()
{
    // prototypePTypeVec
    for (const auto &parameter : parameters) {
        for (const auto &var : parameter->variables()) {
            prototypePTypeVec.emplace_back(var->getType());
        }
    }    
    return this->prototypePTypeVec;
}
...
PTypeSharedPtr FunctionNode::getReturnType() const {return return_type;}

bool FunctionNode::isDefined() const { return (body) ? true : false; }
bool FunctionNode::hasPrameters() const { return (parameters.size() > 0) ? true : false;}
